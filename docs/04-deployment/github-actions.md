...
| Stack deploy hangs / exit 1; смесь старых `:sha` на сервисах | `--resolve-image always` → Swarm manager на VPS резолвит GHCR/Docker Hub и виснет (TLS/`i/o timeout`) | `deploy-dev.sh` → `--resolve-image changed` + retry на registry/connect. Pre-pull GHCR образов обязателен |
