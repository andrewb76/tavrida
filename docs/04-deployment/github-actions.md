...
| Pre-pull fails mid-list / only first image(s) on node | Раньше pull шёл через docker.sock SSH-туннель; длинная серия pull рвала туннель/API | Актуальный `ci-prepull-images.sh` тянет образы **на ноде по SSH** с ретраями. Re-run Deploy (можно `skip_build` + tag SHA) |
