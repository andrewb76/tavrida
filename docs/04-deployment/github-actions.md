...
| Task `Rejected` / `No such image: ghcr.io/…:sha@sha256:…` | Образ private в GHCR; нода не смогла pull (нет/протух registry auth), хотя CI push ок | На VPS: `docker login ghcr.io` + `docker pull …` + `docker service update --with-registry-auth --force …`. CI: шаг `Pre-pull images on Swarm node` (SSH на ноду + login `GITHUB_TOKEN`, не через sock-туннель). Package → link to repo `tavrida` |
| Pre-pull fails mid-list / only first image(s) on node | Раньше pull шёл через docker.sock SSH-туннель; длинная серия pull рвала туннель/API | Актуальный `ci-prepull-images.sh` тянет образы **на ноде по SSH** (parallel, skip-if-present, retries). Step `continue-on-error` — stack deploy всё равно идёт с `--with-registry-auth`. Re-run Deploy (можно `skip_build` + tag SHA) |
| Pre-pull step timeout / slow VPS→ghcr.io | Узкий канал или ретраи на 14 образах | timeout 40m; уже скачанные теги skip; parallel=3. При soft-fail смотри warning в логе и `docker service ps` на Rejected |
| `service ls` показывает `2/1` / `3/1` при `replicas: 1` | Rolling `start-first`: новый таск стартовал, старый не сняли (healthcheck fail → update pause) — несколько Desired=Running на один slot | На `dev` сейчас `order: stop-first` + `failure_action: rollback`. Сброс зомби: `docker service scale NAME=0 && docker service scale NAME=1` |
| Stack deploy hangs / exit 1; смесь старых `:sha` на сервисах | `--resolve-image always/changed` → manager резолвит registry mid-deploy; SSH tunnel blip рвёт `stack deploy` после частичного update | После успешного Pre-pull: `PREPULL_OK=1` → `--resolve-image never`; `STACK_DEPLOY_RETRIES=12` + reconnect poke. Manual: Re-run Deploy (`skip_build`) или `docker service update --image …:sha` для отставших сервисов |
