...
Ранг определяется количеством постов и комментариев на форуме. Показывается в профиле рядом с рейтингом.

### 10.2 Формула

```
rank = getRankForPostCount(postCount + commentCount)
```

| Ранг | Минимум постов/комментариев |
|------|----------------------------|
| newcomer | 0 |
| user | 30 |
| regular | 100 |
...
Ранг **не хранится** — вычисляется на лету по `postCount + commentCount` в `user_rating`.
