...
9. [Медали (награды за достижения)](#-9-медали-награды-за-достижения)
10. [Ранги (уровни участников)](#-10-ранги-уровни-участников)
11. [Штрафы, ограничения и бан](#-11-штрафы-ограничения-и-бан)
12. [Справочник переменных](#-12-справочник-переменных)
13. [Алгоритмы простым языком](#-13-алгоритмы-простым-языком)
14. [Связь с сервисами и событиями](#-14-связь-с-сервисами-и-событиями)
...
| **Медаль** | `medal` | Визуальная награда за достижения; **не влияет** на карму/рейтинг напрямую | enum (7 seeded) |
| **Ранг** | `rank` | Определяется по количеству постов и комментариев; показывается в профиле | newcomer/user/regular/veteran |
...
## 9. Медали (награды за достижения)

### 9.1 Назначение

Медали — **визуальный слой** репутации. Они не влияют на карму или рейтинг напрямую, но показывают вклад участника в сообщество.

### 9.2 Типы медалей

| UUID | Название | Иконка | Описание |
|------|----------|--------|----------|
| `a1b2c3d4-0001-...` | Знак почёта III | `/medals/znak-pocheta-3.svg` | Бронзовая звезда |
| `a1b2c3d4-0002-...` | Знак почёта II | `/medals/znak-pocheta-2.svg` | Серебряная звезда |
| `a1b2c3d4-0003-...` | Знак почёта I | `/medals/znak-pocheta-1.svg` | Золотая звезда |
| `a1b2c3d4-0004-...` | Почётная подвеска | `/medals/pochetnaya-podveska.svg` | Подвеска |
| `a1b2c3d4-0005-...` | Орден почёта | `/medals/orden-pocheta.svg` | Крест |
| `a1b2c3d4-0006-...` | Орден заслуги | `/medals/orden-zaslugi.svg` | Звезда |
| `a1b2c3d4-0007-...` | Знак врача | `/medals/znak-vracha.svg` | Медицинский щит |

> **Иконки:** SVG-файлы в `apps/frontend/public/medals/`, раздаются через nginx как статику (`https://tavridalot.ru/medals/*.svg`). Миграция `1784247300000-MedalIcons` заполняет `icon_url` при деплое.
...
forum.medal:
  ...
  iconUrl     VARCHAR(500)  -- путь к SVG: /medals/*.svg

forum.user_medal:
  userId    UUID PK
  medalId   UUID PK
  awardedAt TIMESTAMPTZ
  awardedBy UUID (admin)
```

### 9.4 Управление

- Админ выдаёт/отзывает медали через `/admin/medals`
- Каталог медалей: CRUD (создание, редактирование, удаление)
- Поиск пользователей для выдачи: autocomplete по username/email
- Пагинация: по 20 пользователей на страницу

### 9.5 Отображение

- В профиле: полный список медалей (иконки 32×32 + названия)
- В теме: медали автора (до 3 иконок 24×24)
- В комментарии: медали автора (до 3 иконок 16×16, inline после имени)
- В админке: медали пользователя (до 3 иконок 16×16, inline после роли/статуса)
- При наведении на иконку — всплывающая подсказка с названием медали

---

## 10. Ранги (уровни участников)

### 10.1 Назначение

Ранг показывает, насколько участник активен на форуме. Это **визуальная метрика** — не влияет на лимиты или права, но помогает понять, насколько опытен пользователь.

### 10.2 Как считается ранг

Ранг считается по **взвешенной сумме** постов и комментариев:

```
score = postCount × postMultiplier + commentCount × commentMultiplier
rank  = getRankForWeightedScore(score)
```

| Параметр | Env var | Default | Описание |
|----------|---------|---------|----------|
| `postMultiplier` | `RANK_POST_MULTIPLIER` | `1.0` | Вес одного поста (темы) |
| `commentMultiplier` | `RANK_COMMENT_MULTIPLIER` | `0.2` | Вес одного комментария |

**Пороги рангов:**

| Ранг | Минимальный score |
|------|-------------------|
| newcomer | 0 |
| user | 30 |
| regular | 100 |
...
| Участник | Посты | Комменты | Score (1.0 / 0.2) | Ранг |
|----------|-------|----------|---------------------|------|
| Новичок | 0 | 5 | 1.0 | newcomer |
| Активный комментатор | 5 | 125 | 30.0 | user |
| Регулярный автор | 50 | 250 | 100.0 | regular |
| Ветеран | 200 | 500 | 300.0 | veteran |

### 10.4 Как это работает простым языком

Каждый написанный на форуме пост (тема) даёт **1 очко**. Каждый комментарий — **0.2 очка** (потому что комментарий требует меньше усилий, чем создание темы).

Когда накопленная сумма очков достигает порога — ранг повышается:
- 30 очков → **user** (ты уже тут как тут)
- 100 очков → **regular** (тебя знают)
- 300 очков → **veteran** (легенда форума)

Коэффициенты можно настраивать через env vars. Если решим, что комментарии важнее — поднимем `RANK_COMMENT_MULTIPLIER`.

### 10.5 Хранение

Ранг **не хранится** — вычисляется на лету по `postCount` и `commentCount` из `user_profile.user_rating`.

### 10.6 Отображение

- В профиле: «Ранг: veteran» (с иконкой)
- В админке: ранг пользователя в таблице
- В теме: ранг автора (опционально)

---

## 11. Штрафы, ограничения и бан
...
## 14. Связь с сервисами и событиями
