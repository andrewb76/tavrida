...
# DRY_RUN=1 pnpm setup:logto-branding   # только payload
```

Скрипт: `PATCH /api/sign-in-exp` (цвет, logo, customCss, hideLogtoBranding) + `PATCH /api/account-center` (customCss для `/account/profile`).  
После деплоя frontend wordmark должен быть доступен по URL выше. **На dev часто остаётся дефолт Logto, пока скрипт не запущен.**

Вручную: Console → Branding / Account center → Custom CSS, **Live preview**.
...
| Swarm bucket | `minio-buckets-init` (`logto-avatars` + media buckets) |
