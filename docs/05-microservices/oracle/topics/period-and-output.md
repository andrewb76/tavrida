...
Вы выбираете, **на сколько месяцев вперёд** считать деньги: 3, 12 или 36. Оракул строит **таблицу по месяцам** и суммирует итог.
...
## Выход расчёта (`OracleResult.months[]`)
...
`POST /admin/oracle/simulate` — поле `periodMonths: number`.
