...
| Кэш `userId → planId` | | ❌ — PostgreSQL read-through без Redis |
...
Plan variables попадают через sync/register от domain-сервисов; временный
central seed также синхронизирует dev-bootstrap каталог.
...
| **PlanVariableTier** | Ячейка матрицы: значение plan variable для конкретного плана |
| **limit** | Числовой потолок; `−1` = без лимита; `null`/disabled tier = deny |
...
### `PlanVariableTier` (`plan_config.plan_variable_tier`)
...
|--------|------|--------|----------|
| POST | `/plan-variables/register` | domain @ startup | Один ключ |
| POST | `/plan-variables/sync` | domain @ startup | Полный манифест; stale для отсутствующих |
| GET | `/plan-variables` | BFF admin | Реестр + tiers |
| PATCH | `/plan-variables/:key` | BFF admin | Изменение matrix row |
| DELETE | `/plan-variables/:key` | admin via BFF | Удаление stale plan variable |
| GET | `/plan-variables/resolve-price?userId=&key=` | BFF | Цена по plan user (`valueType: price`) |
...
| POST | `/subscription/activate` | BFF | Активация / смена тарифа |
| POST | `/subscription/cancel-auto-renew` | BFF | Отключение автопродления |
...
Отсутствующий ключ, `null` limit и disabled tier возвращают `allowed=false` с
машиночитаемым `reason`; только `-1` означает unlimited. `resolvePlanId`
немедленно возвращает Free для `expiresAt <= now`, не ожидая renew CRON.
...
|-----------|-------|-------|
| planned | `subscription.activated` | Runtime publisher пока отсутствует |
| planned | `subscription.expired` | Runtime publisher пока отсутствует |
| planned | `billing.charge_completed` | Runtime consumer пока отсутствует |
