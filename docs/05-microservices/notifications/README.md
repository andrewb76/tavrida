...
> **Статус:** implementing · **Версия:** 0.4 · **Schema:** `notifications` · **Port:** 3010  
> **Платформа:** Novu self-host ([ADR-019](../../03-architecture/adr/019-novu-self-host.md)) · adapter [ADR-004](../../03-architecture/adr/004-notifications-adapter.md)  
...
- Local Novu: `pnpm novu:up` → Dashboard `:4000`, API `:3020`
...
| `id` | UUID PK | — |
| `userId` | varchar | — |
| `workflowId` | varchar | Novu workflow (только из allowlist) |
| `transactionId` | varchar | Novu / mock id |
| `idempotencyKey` | varchar nullable | Unique per `(userId, key)` — дедуп trigger |
...
- Unknown `workflowId` → **400**
- Повтор с тем же `idempotencyKey` → тот же `transactionId` (`deduped: true`)
- Auth: если задан `INTERNAL_SERVICE_TOKEN` — `Authorization: Bearer …` (без env — open для local/dev)
- Без `NOVU_API_KEY` — mock (пишет log); workflow `tag-content` создать в self-host Dashboard (`pnpm novu:up`)
...
| Компонент | Протокол |
|-----------|----------|
| Novu CE (self-host) | HTTP `/v1/events/trigger` → `NOVU_API_URL` (or mock) |
...
| `NOVU_API_URL` | нет | default `http://localhost:3020` (self-host) |
...
- [x] Local compose `novu.local.yml` / `pnpm novu:up` ([ADR-019](../../03-architecture/adr/019-novu-self-host.md))
- [ ] Dashboard: workflow `tag-content` + `NOVU_API_KEY` в `.env.local`
- [ ] Webhook HMAC → update log status
- [ ] Redis relay → BFF `notification.new`
- [ ] Vue Inbox component
- [ ] Merge Novu в `infra.local.yml` / Swarm stack

## 📎 Связанные разделы

- [notifications-analysis](../../03-architecture/notifications-analysis.md)
- [ADR-019](../../03-architecture/adr/019-novu-self-host.md) · [ADR-004](../../03-architecture/adr/004-notifications-adapter.md)
