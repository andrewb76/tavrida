...
Вложения (изображения, PDF и др.) хранятся в JSONB-массиве `attachments` на topic/comment.  
**При создании/обновлении** все вложения автоматически добавляют markdown-ссылки в конец `body`:

- **Изображения** (`image/*`) → `![filename](url)`
- **Файлы** (PDF и др.) → `[filename](url)`

Это позволяет автору перемещать ссылки в любом месте текста. Если ссылки были удалены из body (намеренно или случайно), кнопка **«Восстановить ссылки»** в форме редактирования добавит недостающие ссылки обратно в конец.

**Миграция:** существующие темы/комментарии обновляются SQL-миграцией `SyncAttachmentMarkdown` — markdown-ссылки добавляются в body для всех вложений, отсутствующих в тексте.
