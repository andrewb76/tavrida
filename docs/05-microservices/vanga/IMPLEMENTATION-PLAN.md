...
| 0.5 | [config/vanga.defaults.yaml](../../../config/vanga.defaults.yaml) | ✅ |
...
- [ ] Правки `vanga.defaults.yaml` после совещания (цены, burn, churn)
...
| 1.2 | Загрузчик defaults из YAML | ✅ `loadVangaDefaults()` в BFF |
| 1.3 | `simulate(input) → VangaResult` | ✅ BFF `POST /admin/vanga/simulate` |
| 1.4 | BFF `VangaModule` | ✅ `GET /defaults`, `POST /simulate`, `POST /compare` |
...
- [x] Пример JSON request/response — [vanga-admin-api.md](../../06-api/vanga-admin-api.md)
...
| Шаг | Задача | Выход |
|-----|--------|-------|
| 2.1 | Route `/admin/vanga`, вкладки по [topic-index](./topic-index.md) | ✅ Vue `AdminVangaView` |
| 2.2 | Ползунки из defaults API | ✅ `VangaRangeField` + `GET /defaults` |
...
## Фаза 3 — Сервис `services/vanga` (опционально)
...
| 3.2 | Schema `vanga.saved_scenario` |
...
| 4.4 | Промоция YAML → `scalar-config` domain `vanga.*` |
...
| Риск | Митигация |
|------|-----------|
| Расхождение Vanga vs billing | Единый `monetization-engine`; каталог монетизации |
