...
Если включена программа, часть **вашего** дохода уходит пригласившим. По умолчанию в Vanga **выключена** — чтобы видеть «чистый» сценарий. Флажок показывает разницу.
...
`billing.charge_completed` → referral-rewards (RabbitMQ). **Vanga не слушает MQ** — только формула.
...
- [x] Select модели + условные поля в Vanga UI (комбо, `referral.models[]`)
