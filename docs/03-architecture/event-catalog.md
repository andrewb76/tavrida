...
> **Статус реализации (2026-07-16):** большинство событий — **spec / planned**.  
> **Реальный RMQ сегодня:** `marketplace.order_completed` → `deal-feedback`; `tag.content_tagged` → `subscriptions` → notifications; `invitation.redeemed` ← user-profile (consumers planned).
> **HTTP fallback:** если `RABBITMQ_URL` недоступен, BFF после `PUT /forum/topics/{id}/tags` делает fire-and-forget HTTP `match` → `tag-content` (без await на write path).
...
| | |
|---|---|
| **Status** | **Implemented (RMQ + HTTP fallback)** |
| **Producer** | BFF (`TagEventsPublisher` после `PUT /forum/topics/{id}/tags`) |
...
| | |
|---|---|
| **Status** | **Implemented (RMQ publish)** · consumers planned |
...
| **Notes** | Publish is best-effort (claim succeeds even if RMQ down). Idempotent claim → no second publish. Queue bind for consumers: routing key `invitation.redeemed` on `tavrida-lot.events`. |
| **Payload** | |
...
| tag.content_tagged | | | | | | ✅ RMQ+HTTP | | BFF pub | | — | ✅ consume |
