...
| `chat.message_read` | `message.read` | `chat:{chatId}` | later — ticks |
...
До WS: статус `READ`/`DELIVERED` вычисляется при `GET …/messages` из `chat_member.last_read_at`.
