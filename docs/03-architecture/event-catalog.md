...
> **Реальный RMQ сегодня:** `marketplace.order_completed` → `deal-feedback` (также binding на `auction.completed`, handler stub).  
> **Синхронный HTTP fan-out:** BFF после `PUT /forum/topics/{id}/tags` → subscriptions `match` → notifications `tag-content` (событие `tag.content_tagged` пока **не** публикуется в RMQ).
...
| | |
|---|---|
| **Status** | **Implemented (HTTP sync)** · RMQ planned |
| **Producer** | BFF (после `PUT /forum/topics/{id}/tags`; later: forum → RMQ) |
| **Consumers** | subscriptions `match` → notifications workflow `tag-content` |
| **Payload** | `{ tagId }` (match); notify payload also `{ topicId, contentType, contentId, tagIds[] }` |
...
| **Consumers** | rating (referral recompute), **referral-rewards** (CPA rules) |
| **Payload** | |
...
| tag.content_tagged | | | | | | ✅ via HTTP | | HTTP | | — | ✅ match |
