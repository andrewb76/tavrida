...
> **Реальный RMQ сегодня:** `marketplace.order_completed` → `deal-feedback` (также binding на `auction.completed`, handler stub); `tag.content_tagged` → `subscriptions` → notifications `tag-content`.
