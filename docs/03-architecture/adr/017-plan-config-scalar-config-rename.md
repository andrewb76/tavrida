...
- **Минимум 3 сегмента:** `{domain}.{facet}.{orderedGroup}.{leafName}`
- Facet: `seller`, `bidder`, `author`, `member`, … — когда роль имеет значение
- Numeric prefix в групповом сегменте: `01lot`, `02bid`, … — порядок в admin UI
- Полные правила: [registry-keys.md](../../13-maintenance/registry-keys.md)

Пример: `auction.seller.01lot.activeMax` (было `auction.sellerActiveLots`).
...
- resolve: `GET /internal/v1/charges/resolve?key=auction.seller.08promotion.unitPrice`
