...
- resolve: `GET /internal/v1/plan-variables/resolve-price?userId=…&key=auction.seller.promotion.unitPrice`
...
- ✅ DB schemas: `plan_config`, `scalar_config`; `charge_target` removed from canonical model
- ✅ BFF/callers use `PLAN_CONFIG_URL` and `SCALAR_CONFIG_URL`; legacy aliases retired
