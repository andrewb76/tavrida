...
(историческое имя: `/parameters/register`; runtime alias retired)
...
**Тот же принцип для `scalar-config`:** линейный реестр
`scalar_variable` + `scalar_value`;
`POST /internal/v1/scalar-variables/sync`; `syncStatus: stale` для ключей вне
манифеста; admin `/api/v1/admin/scalar-config/registry` + удаление stale ключей.
