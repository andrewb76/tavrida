# ADR-014: Vanga — прогноз дохода (симулятор)
...
2. **Формулы** — `@tavrida/monetization-engine` ([ADR-015](./015-monetization-engine.md)): pure functions, один source of truth для Vanga **и** platform charge paths.
3. **Vanga** — read-only симулятор; **не** alternative billing path.
4. **Доступ** — admin-only (`/admin/vanga` via BFF).
5. **Деплой по фазам:**
   - Фаза 1: BFF + `packages/monetization-engine` + `config/vanga.defaults.yaml`.
   - Фаза 3: `services/vanga` :3013 при saved scenarios.
6. **Конфиг:** [`config/vanga.defaults.yaml`](../../config/vanga.defaults.yaml) — ranges для совещаний; позже `vanga.*` в scalar-config.
...
См. [vanga-config.md](../../05-microservices/vanga/vanga-config.md).
...
- UI Vanga — не показывать placeholder-цены Basic/Pro в member-facing UI ([PLATFORM-REGISTRY](../../05-microservices/PLATFORM-REGISTRY.md)).

## 🔗 Связанные

- [vanga/README](../../05-microservices/vanga/README.md)
