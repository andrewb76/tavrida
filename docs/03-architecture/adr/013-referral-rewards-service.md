...
Продукт хочет **опциональный денежный слой**: вознаграждение за монетизацию сети (подписки, разовые платежи платформе), с **гибкой настройкой** через `scalar-config` и `plan-config`, без хардкода схем в коде.
...
3. **Rule engine**: правила начисления — JSON в scalar-config `referralRewards.rules`; лимиты и множители по тарифу — plan-config `referralRewards.*`.
