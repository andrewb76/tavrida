...
**Код (каркас):** `billing`, `plan-config`, `scalar-config`, `auction`, `auction-subscriptions` (→ `subscriptions`)  
**Только docs:** `bff`, `rating`, `deal-feedback`, `forum`, `user-profile`, `notifications`, `webhooks`, `marketplace`  
