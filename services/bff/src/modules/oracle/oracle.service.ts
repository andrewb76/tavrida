...
import type { OracleDefaults } from './load-oracle-defaults';
import { buildOracleOverlay, loadOracleDefaults } from './load-oracle-defaults';
import type { ActivityDto, CostsDto, DepositsDto, GrowthDto, ReferralDto, SimulateRequestDto, SubscriptionsDto } from './dto/oracle.dto';
...
export class OracleService {
  private defaultsCache: OracleDefaults | null = null;
  ...
  getDefaults() {
    ...
    return {
      ...
      config,
      overlay: buildOracleOverlay(config),
    };
  }
  ...
  private loadDefaults(): OracleDefaults {
    if (!this.defaultsCache) {
      this.defaultsCache = loadOracleDefaults();
    }
    return this.defaultsCache;
  }

  private toSimulateInput(dto: SimulateRequestDto, defaults: OracleDefaults): SimulateInput {
  ...
  private toGrowthModel(growth: GrowthDto, defaults: OracleDefaults): GrowthModel {
  ...
  private resolvePrices(subscriptions: SubscriptionsDto | undefined, defaults: OracleDefaults): PlanPrices {
  ...
  private readCostDefault(defaults: OracleDefaults, key: string, fallback: number): number {
