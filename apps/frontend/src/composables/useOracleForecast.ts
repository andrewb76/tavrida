...
export function useOracleForecast() {
  ...
  const form = ref<OracleFormState>({
    periodMonths: 12,
    preset: 'base',
    growthModel: 'linear',
  ...
  function initFormFromConfig(cfg: Record<string, unknown>) {
    ...
    form.value = {
      ...
      preset: 'base',
      growthModel: (growth?.defaultModel as OracleFormState['growthModel']) ?? 'linear',
  ...
  function applyPresetMultiplier(
    preset: OracleFormState['preset'],
    base: OracleSimulateRequest,
    ...
    const req = JSON.parse(JSON.stringify(base)) as OracleSimulateRequest;
  ...
  function buildRequest(state = form.value): OracleSimulateRequest {
    const growth =
  ...
  const runSimulate = useDebounceFn(async () => {
    ...
    try {
      if (compareMode.value) {
        const presets: OracleFormState['preset'][] = ['base', 'optimistic', 'pessimistic'];
        ...
        compareResults.value = await compareOracle(scenarios);
        result.value =
        ...
        result.value = await simulateOracle(req);
  ...
  onMounted(async () => {
    try {
      const defaults = await fetchOracleDefaults();
