/** Русские подписи для Oracle UI (без англ. в интерфейсе). */

export const ORACLE_ONE_TIME_LABELS: Record<string, string> = {
...
export const ORACLE_COST_KEY_LABELS: Record<string, string> = {
  hosting: 'Хостинг / VPS',
...
export function oneTimeLabelRu(key: string): string {
  return ORACLE_ONE_TIME_LABELS[key] ?? key.replace(/\./g, ' · ');
}
...
export function costLabelRu(
  ...
  return yamlLabel ?? ORACLE_COST_KEY_LABELS[key] ?? key.replace(/_/g, ' ');
}
